...
Douglas Crockford described some patterns like this (using closures) in *Javascript The Good Parts* but I think he ultimately has [come around to prototypes](http://javascript.crockford.com/prototypal.html).
...
But let's be honest: most of us will not be able to use this for quite some time (though who knows, with [source maps](http://www.html5rocks.com/en/tutorials/developertools/sourcemaps/) we might be able to [compile down to older versions of Javascript](https://github.com/square/es6-module-transpiler) without much loss in debuggability).  So until then we must manually apply sanity when we write Javascript.

Another improvement coming into availability is `"use strict"` -- the ability to apply strict behaviors (both in syntax *and* at runtime) to your code.  This allows you to at least avoid the [problem of forgetting `new`](#nonew) [by setting `this` to undefined when you leave off `new`](https://developer.mozilla.org/en-US/docs/JavaScript/Reference/Functions_and_function_scope/Strict_mode#.22Securing.22_JavaScript)
