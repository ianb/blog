...
There's a bunch of techniques for doing deployments of long-running processes (Zope, Python server, Rails, etc).  A pretty good technique is to do HTTP proxying.  There's some details and conventions I'd like to `see for HTTP <http://blog.ianbicking.org/http-proxying-questions.html>`_, but that's not my concern here.
